...
describe("BalDepositor", function() {
  ...
  before(async () => {
    ...
    const MockVoteProxy_Contract = await ethers.getContractFactory(
      "VoterProxy"
    );
    ...
    const BalDepositorContract = await ethers.getContractFactory(
      "BalDepositor"
    );
    Depositor = await BalDepositorContract.deploy(
      addr1.address,
      addr2.address,
      addr3.address
    );
    ...
    voterContract = await ethers.getContractAt(
      "VoterProxy",
    ...
    mockVbalContract = await ethers.getContractAt(
      "MockVBAL",
      mockVbalContractAddress
    );
    ...
    mockRewardsContract = await ethers.getContractAt(
      "MockRewards",
      mockRewardsContractAddress
    );
    ...
    mockD2DBalContract = await ethers.getContractAt(
      "MockD2DBal",
      mockD2DBalContractAddress
    );
  ...
  describe("Testing deposit function", () => {
    it("Should fail if deposit amount is not sufficient", async () => {
      ...
      await expect(Depositor.deposit(amount)).to.be.revertedWith("!>0");
    });
    ...
    it("Should transfer tokens from sender to the Depositor contract, and defer lock cost to another user", async () => {
      ...
      let setVoterDepositor = await voterContract.setDepositor(
        Depositor.address
      );
      ...
      await coinContract
        .connect(owner)
        .approve(Depositor.address, amountApproved);
      await coinContract
        .connect(owner)
        .increaseAllowance(mockVbalContract.address, deposit_amount_one);

      await Depositor.connect(owner).deposit(deposit_amount_one);

      let vBal_contract_WethBalBalance = await coinContract.balanceOf(
        MockVbal.address
      );
      let rewards_Contract_d2dBalance = await MockD2DBal.balanceOf(
        MockRewards.address
      );

      expect(vBal_contract_WethBalBalance.toString()).to.equal(
        deposit_amount_one.toString()
      );
  ...
  describe("Testing depositAll function", () => {
    it("Should test depositAll function", async () => {
      ...
      await coinContract
        .connect(owner)
        .approve(Depositor.address, OwnerBalance);
      await coinContract
        .connect(owner)
        .increaseAllowance(mockVbalContract.address, OwnerBalance);
