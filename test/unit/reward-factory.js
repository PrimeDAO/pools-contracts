...
describe("RewardFactory", function () {
  ...
  context("Active rewards", async function () {
    ...
    it("reverts if user is unauthorized to add reward", async function () {
      ...
      await expect(rewardFactoryContract.addActiveReward(addressOne, 1)).to.be.revertedWith("Unauthorized()");
    });
    ...
    it("reverts if user is unauthorized to add access", async function () {
      ...
      await expect(rewardFactoryContract.setAccess(anotherUser.address, true)).to.be.revertedWith("Unauthorized()");
    });
    ...
    it("removes reward", async function () {
      ...
      // add extra rewards
      await expect(rewardFactoryContract.connect(anotherUser).addActiveReward(addressOne, pid)).to.emit(
        rewardFactoryContract,
        "ExtraRewardAdded"
      );
      await expect(rewardFactoryContract.connect(anotherUser).addActiveReward(addressOne, pid + 1)).to.emit(
        rewardFactoryContract,
        "ExtraRewardAdded"
      );
      await expect(rewardFactoryContract.connect(anotherUser).addActiveReward(addressOne, pid + 2)).to.emit(
        rewardFactoryContract,
        "ExtraRewardAdded"
      );
      await expect(rewardFactoryContract.connect(anotherUser).addActiveReward(addressOne, pid + 3)).to.emit(
        rewardFactoryContract,
        "ExtraRewardAdded"
      );
  ...
  context("Create pools", async function () {
    it("creates bal rewards pool", async function () {
      ...
      await expect(rewardFactoryContract.createBalRewards(1, addressOne)).to.be.revertedWith("Unauthorized()");

      // if this doesn't revert the pool is created
      await expect(rewardFactoryContract.connect(operator).createBalRewards(1, addressOne)).to.emit(
        rewardFactoryContract,
    ...
    it("creates token rewards pool from operator", async function () {
      ...
      await expect(
        rewardFactoryContract.createTokenRewards(addressOne, addressTwo, operator.address)
      ...
      await expect(
        rewardFactoryContract.connect(operator).createTokenRewards(addressOne, mainPoolAddress, operator.address)
    ...
    it("creates token rewards pool from rewardAccess role", async function () {
      ...
      await expect(
        rewardFactoryContract.connect(anotherUser).createTokenRewards(addressOne, mainPoolAddress, operator.address)
      ).to.emit(rewardFactoryContract, "VirtualBalanceRewardPoolCreated");
    });
  });
});
