...
describe("RewardFactory", function() {
    ...
    context("Active rewards", async function() {
        it("adds reward", async function() {
            ...
            await expect(
                rewardFactoryContract
                    .connect(anotherUser)
                    .addActiveReward(addressOne, pid)
            )
        ...
        it("reverts if user is unauthorized to add reward", async function() {
            ...
            await expect(
                rewardFactoryContract.addActiveReward(addressOne, 1)
        ...
        it("reverts if user is unauthorized to remove reward", async function() {
            ...
            await expect(
                rewardFactoryContract.removeActiveReward(ZERO_ADDRESS, 1)
            ).to.be.revertedWith("Unauthorized()");
        });
        ...
        it("gets rewards count", async function() {
            ...
            expect(
                await rewardFactoryContract
                    .connect(anotherUser)
                    .addActiveReward(ZERO_ADDRESS, pid)
            );

            expect(
                await rewardFactoryContract.activeRewardCount(addressOne)
            ).to.equal(1);
        });
        ...
        it("removes reward", async function() {
            ...
            await expect(
                rewardFactoryContract
                    .connect(anotherUser)
                    .addActiveReward(addressOne, pid + 1)
            ).to.emit(rewardFactoryContract, "ExtraRewardAdded");
            await expect(
                rewardFactoryContract
                    .connect(anotherUser)
                    .addActiveReward(addressOne, pid + 2)
            ).to.emit(rewardFactoryContract, "ExtraRewardAdded");
            await expect(
                rewardFactoryContract
                    .connect(anotherUser)
                    .addActiveReward(addressOne, pid + 3)
            ).to.emit(rewardFactoryContract, "ExtraRewardAdded");
            ...
            await expect(
                rewardFactoryContract
                    .connect(anotherUser)
                    .removeActiveReward(addressOne, pid)
            )
    ...
    context("Create pools", async function() {
        it("creates bal rewards pool", async function() {
            ...
            await expect(
                rewardFactoryContract.createBalRewards(1, addressOne)
            ).to.be.revertedWith("Unauthorized()");
            ...
            await expect(
                rewardFactoryContract
                    .connect(operator)
                    .createBalRewards(1, addressOne)
            ).to.emit(rewardFactoryContract, "BaseRewardPoolCreated");
        });
        ...
        it("creates token rewards pool from operator", async function() {
            ...
            await expect(
                rewardFactoryContract.createTokenRewards(
                    addressOne,
                    addressTwo,
                    operator.address
                )
            ...
            const tx = await rewardFactoryContract
                .connect(operator)
                .createBalRewards(1, addressOne);
