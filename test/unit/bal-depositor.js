...
describe('unit - Contract: BalDepositor', async () => {
  ...
  context('» setFeeManager testing', () => {
    it('sets the fee manager', async () => {
      ...
      expect(await balDepositor.feeManager()).to.equal(root.address);
    });
  });
  context('» setFees testing', () => {
    it('allows feeManager to set a new lockIncentive', async () => {
      await balDepositor.setFees(incentiveInRange);
      expect(await balDepositor.lockIncentive()).to.equal(incentiveInRange);
    });
  ...
  context('» deposit testing', () => {
    it('fails if deposit amount is too small', async () => {
      ...
      await expect(balDepositor.deposit(0, true, voterProxy.address)).to.be.revertedWith('InvalidAmount()');
    });
