...
describe("VoterProxy", function () {
    ...
    it('reverts if no stash access', async function () {
        await expect(voterProxy.connect(anotherUser)['withdraw(address)'](B50WBTC50WETH.address))
            .to.be.revertedWith('Unauthorized()')
    });
    ...
    context('Withdraw', async function () {
        ...
        context('withdraw(address,address,uint256)', async function () {
            it('withdraws amount', async function () {
                ...
                expect(await voterProxy.connect(anotherUser)['withdraw(address,address,uint256)'](B50WBTC50WETH.address, gauge.address, oneHundredFiftyEther))
