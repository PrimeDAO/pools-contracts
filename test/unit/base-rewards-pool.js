...
describe('unit - BaseRewardPool', function () {
  const setupTests = deployments.createFixture(async () => {
    ...
    setup.operatorFactory = await ethers.getContractFactory('ControllerMock');
  ...
  before('setup', async function () {
    ...
    expect(await baseRewardPool.NEW_REWARD_RATIO()).to.equal(NEW_REWARD_RATIO);
    expect(await baseRewardPool.extraRewardsLength()).to.equal(ZERO);
    expect(await baseRewardPool.extraRewardsLength()).to.equal(ZERO);
    expect(await baseRewardPool.lastTimeRewardApplicable()).to.be.equal(BigNumber.from(ZERO));
    expect(await baseRewardPool.rewardPerToken()).to.be.equal(BigNumber.from(ZERO));
  });
  ...
  context('Stake', async function () {
    ...
    it('reverts on invalid stake amount for somebody else', async function () {
      ...
      await expect(baseRewardPool.connect(root).stakeFor(anotherUser.address, 0)).to.be.revertedWith('InvalidAmount()');
    });
  ...
  it('changes ratio by queueing new rewards multiple times', async function () {
    ...
    await expect(operator.queueNewRewards(rewardAmount)).to.emit(baseRewardPool, 'RewardAdded').withArgs(rewardAmount);
  ...
  it('changes ratio by queueing new rewards multiple times queuedRation > NEW_REWARD_RATIO', async function () {
    ...
    expect(await baseRewardPool.queuedRewards()).to.equal(BigNumber.from(ONE_WEEK.toString()).mul(100));
  });

  it('queues and gets the reward', async function () {
    if (process.env.BLOCKCHAIN_FORK) {
      // fails on kovan fork
      this.skip();
    }
