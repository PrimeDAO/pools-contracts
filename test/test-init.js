...
const getTokens = async (setup) => {
  ...
  const GaugeControllerFactory = await ethers.getContractFactory("GaugeControllerMock", setup.roles.root);
...
const balDepositor = async (setup) => {
  ...
  return await balDepositor.deploy(setup.tokens.WethBal.address, staker.address, setup.tokens.D2DBal.address);
};

const controller = async (setup) => {
  const controller = await ethers.getContractFactory("Controller", setup.roles.root);
  const staker = setup.voterProxy;
  const minter = staker;
...
const getVoterProxy = async (setup) => {
  ...
  return await VoterProxy.deploy(mintr.address, wethBal.address, bal.address, veBal.address, gaugeController.address);
};
